...
make.net <- function(df) {
  expr <- t(df)[,goodGenes(t(df), verbose=0)]
  expr[is.na(expr)] <- 0
...
cyt = exportNetworkToCytoscape(enzyme.tot, edgeFile = paste0("CytoscapeInput-edges-", "Fungi", ".txt"), nodeFile = paste("CytoscapeInput-nodes-", "Fungi", ".txt"), weighted=T, threshold=0, nodeNames=colnames(enzyme.tot))
