...
make.enzyme.net <- function(adj, enzyme.names){
  ...
  for(i in enzyme.names){
    ...
    for(j in enzyme.names){
      ...
      if(!is.na(tot)){
        if(i==j){
          enzyme.net[i, j] <- (sum(adj[rownames(adj)[selected_i], colnames(adj)[selected_j]])-table(enzymes)[i])/tot
        } else {
...
#table(unlist(str_split(raw_data$Enzyme.type[raw_data$Enzyme.type!=""], ";")))
...
summ_data <- raw_data %>%
  ...
  spread(Organism, N) %>%
  rowwise() %>%
  mutate(tot=rowSums(.[2:6], na.rm=T))
...
sum(enzymes.Ps[upper.tri(enzymes.pruned,diag=T)]==1)
