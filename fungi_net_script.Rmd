...
n <- discretize(enzyme.tot, method="interval", labels=F, breaks=101, include.lowest=T)-1
n <- matrix(n, ncol=sqrt(length(n)))
colnames(n) <- colnames(enzyme.tot)
rownames(n) <- rownames(enzyme.tot)
write.csv(n, file=paste0(wd, "/results/enzyme_tot.csv") , quote=F)


```

Compute significnace with python script

```{r import significance}

enzyme.Ps <- read.csv(paste0(wd, "/results/enzyme_pruned.csv"), row.names=1, header=T)
enzyme.Ps <- ifelse(enzyme.Ps<0.05, 1, 0)
enzyme.pruned <- enzyme.tot * enzyme.Ps

sum(enzyme.Ps[upper.tri(enzyme.pruned,diag=T)]==1)
...
g_pruned <- graph_from_adjacency_matrix(enzyme.pruned, mode="undirected", weighted=T)
